    ...
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          while (Date.now() < stop) {
            const text = (await MessageV2.parts(msg.id)).find((part): part is MessageV2.TextPart => part.type === "text")
        ...
        const text = (yield* Effect.promise(() => MessageV2.parts(msg.id))).find((part): part is MessageV2.TextPart => part.type === "text")
