...
export const InstanceRoutes = (upgrade: UpgradeWebSocket, opts?: CorsOptions): Hono => {
  ...
  if (Flag.OPENCODE_EXPERIMENTAL_HTTPAPI) {
    ...
    app.post(SessionPaths.newBranch, (c) => handler(c.req.raw, context))
    app.post(SessionPaths.checkoutBranch, (c) => handler(c.req.raw, context))
    app.get(SessionPaths.branches, (c) => handler(c.req.raw, context))
    app.get(SessionPaths.log, (c) => handler(c.req.raw, context))
    app.post(SessionPaths.sql, (c) => handler(c.req.raw, context))
    app.get(SessionPaths.context, (c) => handler(c.req.raw, context))
    app.post(SessionPaths.diffStat, (c) => handler(c.req.raw, context))
